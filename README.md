...
To generate predictions using the pre-trained HAFT model and create a Codabench-ready submission:
```bash
python "HAFT FACTSHEET/run.py"
```
- **Checkpoint**: Uses `HAFT FACTSHEET/checkpoints/haft_large_best_psnr.pth`.
- **Input**: The script reads from `HAFT FACTSHEET/dataset/Bokeh_NTIRE2026`.
- **Output**: Individual prediction images and the required `readme.txt` are stored in the `HAFT FACTSHEET/outputs/` directory.
