...
To generate predictions using the pre-trained HAFT model and create a Codabench-compliant submission:
```bash
python "HAFT FACTSHEET/run.py"
```
*Note: The script utilizes the checkpoint at `checkpoints/haft_large_best_psnr.pth`. Input images are read from the specified directory, and results are stored in the `outputs/` folder. A submission archive is automatically generated in the root directory.*
