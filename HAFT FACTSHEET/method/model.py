...
class Bokehlicious(Module):
    ...
    def __init__(self,
        ...
        # Determine if we are using the HAFT CoC map (usually adds 1 channel)
    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Conv2d):
            # The CoC Zero-Init Trick (Mr. X)
            # If this is the in_stage conv, and we added the CoC map,
            # we must initialize the CoC channel weights to exactly 0.0.
            # This prevents the uninitialized channel from destroying pre-trained RGB filters.
            if m is self.in_stage and self.use_coc_map:
                # Truncated normal for everything else (or whatever default)
                # But realistically, if loaded from pretrained, these get overwritten anyway.
                # Setting the NEW channel to 0.0 ensures it starts invisible.
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                if m.bias is not None:
                    nn.init.constant_(m.bias, 0)
                
                # The CoC map is the very last channel added:
                with torch.no_grad():
                    m.weight[:, -1, :, :] = 0.0
    ...
    def forward(self, source: Tensor, bokeh_strength: Tensor = None, pos_map: Tensor = None,
                bokeh_strength_map: Tensor = None, coc_map: Tensor = None, **kwargs):
